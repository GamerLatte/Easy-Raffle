...
public sealed class SessionStore : IDisposable
{
	...
	public SessionStore(string baseDir)
	{
		...
		sessionPath = Path.Combine(baseDir, "RafflerSession.json");
		backupDir = Path.Combine(baseDir, "RafflerSession.backups");
	...
	private void LoadOrCreate()
	{
		...
		catch
		{
			try
			{
				...
				File.Copy(sessionPath, Path.Combine(backupDir, "RafflerSession.corrupt_" + text + ".json"), overwrite: true);
			}
			catch
			{
			}
	...
	private void SaveAtomic()
	{
		lock (gate)
		{
			...
			try
			{
				...
				File.WriteAllText(Path.Combine(backupDir, "RafflerSession_" + text2 + ".json"), contents, Encoding.UTF8);
				FileInfo[] files = new DirectoryInfo(backupDir).GetFiles("RafflerSession_*.json");
