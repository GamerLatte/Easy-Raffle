...
public sealed class Plugin : IDalamudPlugin, IDisposable
{
	...
	public readonly WindowSystem WindowSystem = new WindowSystem("Easy Raffle");
	...
	private EasyRaffleLiteWindow EasyRaffleLiteWindow { get; init; }
	...
	public Plugin()
	{
		...
		string raffleImgArg = Path.Combine(PluginInterface.AssemblyLocation.Directory.FullName, "easyraffle.png");
		ConfigWindow = new ConfigWindow(this);
		TicketListWindow = new TicketListWindow(this);
		MainWindow = new MainWindow(this, raffleImgArg, TicketListWindow, null);
		EasyRaffleLiteWindow = new EasyRaffleLiteWindow(this, MainWindow, TicketListWindow);
		...
		WindowSystem.AddWindow((Window)(object)EasyRaffleLiteWindow);
		...
		{
			HelpMessage = "Open Easy Raffle UI."
		});
		CommandManager.AddHandler("/rafflerlite", new CommandInfo((HandlerDelegate)delegate
		{
			((Window)EasyRaffleLiteWindow).Toggle();
		})
		{
			HelpMessage = "Open Easy Raffle Lite"
		});
		...
		Log.Information($"=== Easy Raffle plugin {PluginInterface.Manifest} loaded! ===", Array.Empty<object>());
	}
	...
	{
		// Authorization check removed - all users can access
